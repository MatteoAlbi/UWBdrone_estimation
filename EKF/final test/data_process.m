...
%% Real measurements
load("data_collected\mavic1.mat");
true_pos = squeeze(Measurement4.RigidBodies.Positions)/1000;
true_rot = squeeze(Measurement4.RigidBodies.Rotations);
true_angles = squeeze(Measurement4.RigidBodies.RPYs);

%% Corrupted measurements
data = readcell("data_collected\mavic1.csv");
n = length(data);
...
t_true = (0:1:Measurement4.Frames-1) ./ Measurement4.FrameRate;
...
for k=3:n
    ...
    if length(tmp) == 4
        ...
        %imu data
        acc = tmp(1:3); %[m/s^2]
        buf_acc = [buf_acc(:,1:7), reshape(acc,3,1)];
        mag = tmp(4:6); %[gauss]
        gyro = tmp(7:9)/180*pi; %[rad/s]
        att_t = tmp(end); %[s]
...
legend('pos_x_est', 'pos_y_est', 'pos_x', 'pos_y','Location', 'best');
xlabel('t [s]');
ylabel('pos [m]');
title("position");
...
% plot(t, X_pos(3,:));
% plot(t, X_pos(4,:));
legend('vel_x_est', 'vel_y_est', 'Location', 'best');
xlabel('t [s]');
ylabel(['v [m/s]']);
title("velocity");
