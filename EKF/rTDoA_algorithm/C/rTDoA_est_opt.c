...
bool rTDoA(uint64_t ts_param[NUM_ANCH][4], double *x_t){
    ...
    //row 82
    //pseudoinverse of A
    if(! pseudoinverse(tmp_m1_flat, tmp_m2_flat, NUM_ANCH-1, 4) ){
    ...
    if(! pseudoinverse(tmp_m1_flat, tmp_m2_flat, NUM_ANCH-1, 3) ) {
        printf("Matrix A non invertible\n");
        return false;
    }
    ...
    for(i=0; i<3; i++){
        x_t[i] = tmp_v3[i];
        for(j=0; j<NUM_ANCH-1; j++){
            x_t[i] += tmp_m2_flat[i * (NUM_ANCH-1) + j]*(tmp_v1[j]-tmp_v2[j]);
        }
    }
