...
static ekf_data_t acc_unc[3];
static ekf_data_t mag_unc[3];
static ekf_data_t soft_iron[9];
...
static ekf_data_t X_att_prev[4];
static ekf_data_t P_att_prev[16];
#ifndef NO_POS
...
static ekf_data_t dt = 0.01; // interval between cycles of acc measurements
...
uint32_t ekf_get_acc_mag_data(const ekf_data_t * const acc, const ekf_data_t * const mag){
    ...
    // get acc data
    memcpy(acc_data, acc, sizeof(ekf_data_t)*3);

    // acc norm [mg]
    ekf_data_t tmp = norm2(acc_data, 3);
    // compute acc_norm_sigma
    acc_norm_sigma = 0;
    for(uint8_t i=0; i<3; i++) acc_norm_sigma += acc_unc[i] * powf(acc_data[i]/tmp, 2); // uncertainty of acc norm
    // standard deviation
    acc_norm_sigma = sqrtf(acc_norm_sigma) * EKF_SIGMA_99; // prob = 1 - 0.99 of detecting false ext acc
    acc_norm_dev = fabsf(tmp - EKF_G_MAGNITUDE);
    ...
    memcpy(mag_data[mag_buff_iter], mag, sizeof(ekf_data_t)*3);
...
void norm_acc(ekf_data_t *const an, ekf_data_t *const Ra){
    ...
    // compute unc
    for(uint8_t i=0; i<3; i++) ekf_data_buf[i] = (ekf_data_t)acc_unc[i];
...
void norm_mag(ekf_data_t *const mn, ekf_data_t *const Rm, uint8_t iter){
    ...
    // compute unc
    for(uint8_t i=0; i<3; i++) ekf_data_buf[i] = (ekf_data_t)mag_unc[i];
    v2diag_m(ekf_data_buf, Rm, 3);
    // // calibration part
    // for(uint8_t i=0; i<3*3; i++) v_tmp[i] = (ekf_data_t)soft_iron[i];
...
void ekf_TRIAD(void){
    ...
    memcpy(&data_log[4], z, sizeof(ekf_data_t)*5);
...
void ekf_att_update(void){
    ...
    // Kalman steps
    quadratic_form(Jh, P_att, S, 5, 4);
    //display_flat_matrix(S, 4, 4);
    for(uint8_t i=0; i<5*5; i++) S[i] += Rz[i]; // S
    //display_flat_matrix(S, 4, 4);
    // bool inv = inverse(S,S,5);
    // if(inv) printf("S not invertible");


    //display_flat_matrix(S, 4, 4);
    matrix_mult_T(P_att, Jh, v_tmp, 4, 5, 4); // v_tmp <- P_att * Jh.t
    // matrix_mult(v_tmp, S, W, 4, 5, 5); 
    matrix_r_divide(v_tmp, S, W, 4, 5, 5);// W = P_att * Jh.t / S

    // X_att
    matrix_mult(W, inn, v_tmp, 4, 1, 5); // v_tmp <- W * inn
    for(uint8_t i=0; i<4; i++) X_att[i] += v_tmp[i];
    normalize(X_att, 4);

    // P_att
    matrix_mult(W, Jh, v_tmp, 4, 4, 5); // v_tmp <- W * Jh
    for(uint8_t i=0; i<4*4; i++) v_tmp[i] = -v_tmp[i]; // v_tmp <- - W * Jh ;
    for(uint8_t i=0; i<4; i++) v_tmp[i*5] += 1.0; // v_tmp <- I - W * Jh;
...
uint32_t ekf_init(ekf_data_t *const ret, const ekf_data_t *const au, const ekf_data_t *const mu, const ekf_data_t *const si,
                    const ekf_data_t * const acc, const ekf_data_t * const mag){
    uint32_t err;

    memcpy(acc_unc, au, sizeof(ekf_data_t)*3);
    memcpy(mag_unc, mu, sizeof(ekf_data_t)*3);
    memcpy(soft_iron, si, sizeof(ekf_data_t)*9);
    ...
    err = ekf_get_acc_mag_data(acc, mag);

    if(err) {/* Manage error */}
    else ekf_init_att(); // init attitude

    memcpy(X_att_prev, X_att, sizeof(ekf_data_t)*4);
    memcpy(P_att_prev, P_att, sizeof(ekf_data_t)*16);
    
#ifdef DEBUG_ON
    memcpy(data_log, X_att, sizeof(ekf_data_t) * 4);
#endif
...
#ifdef DEBUG_ON
    memcpy(ret, data_log, sizeof(ekf_data_t) * 16);
#endif
...
void ekf_step(ekf_data_t *const ret, const ekf_data_t * const acc, const ekf_data_t * const mag){
    ...
    // ekf_data_t update_diff = 0;
    // for(uint8_t i=0; i<4; i++) update_diff += fabsf(X_att[i] - X_att_prev[i]);
    // if(update_diff > 2){ 
    //     memcpy(X_att, X_att_prev, sizeof(ekf_data_t)*4);
    //     memcpy(P_att, P_att_prev, sizeof(ekf_data_t)*16);
    //     // float alpha = 0.1;
    //     // for(uint8_t i=0; i<4; i++) X_att[i] = alpha * X_att[i] + (1-alpha) * X_att_prev[i];
    //     // normalize(X_att, 4);
    //     // for(uint8_t i=0; i<4*4; i++) P_att[i] = alpha * P_att[i] + (1-alpha) * P_att_prev[i];
    // }
    // else{
    //     memcpy(X_att_prev, X_att, sizeof(ekf_data_t)*4);
    //     memcpy(P_att_prev, P_att, sizeof(ekf_data_t)*16);
    ...
    memcpy(&data_log[9], z, sizeof(ekf_data_t) * 3);
    data_log[12] = mag_buff_iter;
#endif
...
#ifdef DEBUG_ON
    memcpy(ret, data_log, sizeof(ekf_data_t) * 16);
    for(int i=0; i<16; i++) data_log[i] = 0;
#endif
