    ...
    if (seq_num == 6){
        ...
        *tick = xTaskGetTickCount();
        ...
        /* check for anomalies in data, 2 cases:
            pos_data == 0
            pos_data very high
          this is observed by empirical data, the function to estimate position sometiems
          compute wrong values probably due to errors in uwb transmissions. Anomalies verify 
          an all 3 axis, thus it is enough to check one
        */
        if(rtdoa_buf[0] == 0.0 || rtdoa_buf[0] > TDOA_MAX_ACCEPTED_POS){
            return RTDOA_COMP_ERR;
        }
