...
// Storage for LIS2 bypass reading
static tsFifoElement accDataBypass;

void vLIS2_EnableBypass (void)
{
	// Set semaphore, no FIFO event yet
	boInterruptEvent = false;
  ...
  // Enable X,Y,Z sensors and set a default sample rate
	...
	// Selecting 12-bit resolution, range +/- 2g
	vTWI_Write(CTRL_REG4, (HIRES_MODE | FS_2G | BDU_ENABLE));
                            //high resolution, range +- 2g -> 1mg/digit
	eRsolution = mode_12bit;
	u8SignExtend = 4;
...
/*!
* @brief Function that checks for new data available from the sensor
* and if new data is available it load them in accDataBypass.
* Then copies accDataBypass in passed variables.
*/
void vLIS2_TaskBypass (void)
{
        uint8_t u8StatusRegister;
        uint8_t u8DataAvailable;
 
        // Check if new data is available
        vTWI_Read(STATUS_REG_ADD, &u8StatusRegister);
        u8DataAvailable = u8StatusRegister & STATUS_ZYXDA;
        
        // If the flag ZYXDA in STATUS_REG (27h) is high new
        ...
        if(u8DataAvailable)
        {
                ...
                u8LoData = u8Reading;
                vTWI_Read(OUT_X_HI, &u8Reading);
                accDataBypass.u16_X = ( (u8Reading << 8) | u8LoData);

                // Y-axis	
                vTWI_Read(OUT_Y_LO, &u8Reading);
                u8LoData = u8Reading;
                vTWI_Read(OUT_Y_HI, &u8Reading);
                accDataBypass.u16_Y = ( (u8Reading << 8) | u8LoData);

                // Z-axis
                vTWI_Read(OUT_Z_LO, &u8Reading);
                u8LoData = u8Reading;
                vTWI_Read(OUT_Z_HI, &u8Reading);
                accDataBypass.u16_Z = ( (u8Reading << 8) | u8LoData);
                
                // Indicate end of new data reading
