...
// varaible for ext acc detection and estimation
static uint8_t ext_acc_counter = 0;
static uint8_t acc_det_threshold = 5;
static ekf_data_t ca = EKF_CA/2;
static ekf_data_t ext_acc[3];
...
uint32_t ekf_get_acc_mag_data(void){
    ...
    if(acc_norm_dev > acc_norm_sigma){ 
        ext_acc_counter = MIN(ext_acc_counter++, acc_det_threshold);
        ...
        ext_acc_counter = MAX(ext_acc_counter--, 0);
        // mag_buff_iter = 0;
    }

    if(ext_acc_counter == acc_det_threshold){
        if(mag_buff_iter == 1){
            // shift registers
            memcpy(mag_data[0], mag_data[1], sizeof(ekf_data_t) * 3);
            //mag_tick[0] = mag_tick[1];
...
void norm_acc(ekf_data_t *const an, ekf_data_t *const Ra){
    ekf_data_t v_tmp[9], tmp, acc_corrected[3];

    ekf_data_t norm_acc_data = norm2(acc_data, 3);
    ekf_data_t norm_ext_acc = norm2(ext_acc, 3);

    // compensate external acc
    for(uint8_t i=0; i<3; i++) acc_corrected[i] = acc_data[i] - ext_acc[i] * ca;
    // normalize acc
    tmp = norm2(acc_corrected, 3);
    for(uint8_t i=0; i<3; i++) an[i] = acc_corrected[i] / tmp;

    // -- compute uncertainty
    // sensor uncertainty
    for(uint8_t i=0; i<3; i++) ekf_data_buf[i] = (ekf_data_t)acc_config->unc[i];
    v2diag_m(ekf_data_buf, Ra, 3);
    // ext_acc uncertainty (before or after normalization?)
    tmp = 0;
    for(uint8_t i=0; i<3; i++) tmp += fabsf(ext_acc[i]);
    tmp = pow(ca * norm_ext_acc,2) + fabsf(pow(norm_acc_data, 2) - EKF_G_MAGNITUDE) / tmp;
    for(uint8_t i=0; i<3; i++) Ra[i*4] += ext_acc[i] * tmp;
...
void ekf_att_update(void){
    ...
    // ext acc
    ekf_quat2rotM(X_att, v_tmp);
    for(uint8_t i=0; i<3; i++) ext_acc[i] = acc_data[i] - EKF_G_MAGNITUDE * v_tmp[2*3 + i];
